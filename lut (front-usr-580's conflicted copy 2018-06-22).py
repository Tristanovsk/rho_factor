...
class lut:
    ...
    def generate_vza_compliant(self, vza_grid= (np.array([30,40,50]))):
        '''
        ...
        for iws in range(Nws):
            for iaot in range(Naot):
                for iwl in range(self.wl.__len__()):
                    for isza in range(self.sza.__len__()):
                        for iazi in range(self.azi.__len__()):
                            Lg[iws, iaot, iwl,isza,iazi,:] = self.interpn_lut([self.grid_lut[-1]], self.Lg[iws, iaot, iwl,isza,iazi], vza_grid)
                            Lsurf[iws, iaot, iwl] = self.interpn_lut([self.grid_lut[-1]], self.Lsurf[iws, iaot, iwl,isza,iazi], vza_grid)
                            Lsky[iws, iaot, iwl] = self.interpn_lut([self.grid_lut[-1]], self.Lsky[iws, iaot, iwl,isza,iazi], vza_grid)
        self.Lg = Lg
        self.Lsurf = Lsurf
        self.Lsky = Lsky
