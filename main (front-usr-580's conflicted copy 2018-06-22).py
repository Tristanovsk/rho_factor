...
df_ = df[df.sza.isin([0, 10,20, 30, 40, 50, 60, 70, 80]) & (df.azi.isin([90,135]))]
...
g = sns.FacetGrid(df_[(df_.aot == 0.5)], col="sza", hue="wind", condition="vza", col_wrap=3, size=1.5, legend_out = True)
...
lutc = lut(aero_coarse_file, ws=[0, 2, 5, 10])
lutc.generate_aeronet_compliant()
